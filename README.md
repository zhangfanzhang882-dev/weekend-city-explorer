...
6. **按天拆分**：跨天行程为每一天分别安排地点，每天单独做地理校验与营业时间校验（同一场馆可能周二闭馆、周三开放）
7. **环境与住宿**：并行取节假日、空气质量；跨天行程按第一天最后一站坐标搜附近住宿。三者互不依赖，任一失败都不影响路线
...
**跨天行程**：`date` 与 `endDate` 不同即视为跨天。每天安排 3 个地点（3 天及以上每天 2 个），站点带 `dayIndex` 与 `date`，路线带 `dayPlan` 每日概要。单日阈值为相邻 6km / 跨度 12km；跨天每天放宽到 8km / 16km，因为跨天本就不要求一天走完。
...
| 住宿推荐 | 高德住宿类 POI 周边搜索 | 跨天行程才出现，以第一天最后一站为圆心 2.5km，可搜索与排序 |
